...
export default function SolarOwnerDetails() {
  ...
  const loadData = async () => {
    try {
      ...
      const res = await fetch(
        `http://localhost:8088/EV/api/admins/solar-owners?${params.toString()}`
      );
  ...
  const downloadCsv = () => {
    ...
    window.open(
      `http://localhost:8088/EV/api/admins/solar-owners/csv?${params.toString()}`,
      "_blank"
    );
  };
  ...
  return (
    <>
      {/* ✅ SAME WRAPPER AS EV OWNER DETAILS (important) */}
          ...
          <div className="overflow-hidden bg-white border shadow-lg border-blueGray-100 rounded-2xl">
            ...
            <div className="px-6 py-5 border-b border-blueGray-100">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                ...
                {/* ✅ responsive controls (same as EV) */}
            ...
            <div className="w-full overflow-x-auto">
              <table className="w-full min-w-[1000px] text-sm">
                ...
                <tbody className="divide-y divide-blueGray-100">
                  {loading ? (
                    <tr>
                      <td className="px-6 py-8 text-blueGray-500" colSpan={6}>
                        Loading...
                      </td>
                    </tr>
                  ) : filtered.length === 0 ? (
            ...
            <div className="px-6 py-4 text-xs bg-white border-t text-blueGray-400 border-blueGray-100">
              Showing <span className="font-bold">{filtered.length}</span> records
              • Selected Month <span className="font-bold">{month}</span>
            </div>
          </div>
        </div>
      </div>
